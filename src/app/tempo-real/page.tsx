...
export default function RealtimePage() {
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-[#020403] text-white">
      ...
      <div className="relative z-10 mx-auto flex max-w-[1800px] flex-col gap-4 px-3 py-4 sm:px-5 lg:px-8 2xl:px-10">
        <section className="relative overflow-hidden rounded-2xl border border-lime-300/20 bg-white/[0.055] p-4 shadow-[0_0_70px_rgba(132,204,22,0.12)] backdrop-blur sm:p-5 xl:p-6">
          ...
          <div className="relative z-10 grid gap-4 xl:grid-cols-[auto_1fr_auto] xl:items-center">
            ...
            <div className="text-center xl:text-left">
              ...
              <h1 className="mt-3 text-4xl font-black uppercase leading-none text-white sm:text-5xl xl:text-6xl 2xl:text-7xl">
        ...
        <section className="flex flex-col gap-3 rounded-xl border border-white/10 bg-black/45 p-3 backdrop-blur xl:flex-row xl:items-center xl:justify-between">
            ...
            <div className="flex min-w-max gap-2">
              {phaseTabs.map((tab) => (
                <button
                  ...
                  className={cn(
                    "h-12 rounded-md border px-4 text-xs font-black uppercase tracking-normal transition sm:px-5 xl:text-sm",
                    activePhase === tab.id
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs font-black uppercase text-slate-400">
            <span className="rounded-md border border-white/10 bg-white/[0.04] px-3 py-2">Atualizado automaticamente</span>
            <span className="rounded-md border border-white/10 bg-white/[0.04] px-3 py-2">Última leitura: {updatedAt}</span>
          </div>
        </section>
...
function GroupRealtimeCard({
  ...
  return (
    <article className="relative min-h-[25rem] overflow-hidden rounded-xl border border-white/10 bg-slate-950 p-3 shadow-[0_0_34px_rgba(255,255,255,0.045)]">
      <div className={`pointer-events-none absolute inset-0 bg-gradient-to-br ${group.theme.gradient} opacity-20`} />
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_50%_0%,rgba(255,255,255,0.18),transparent_30%),linear-gradient(180deg,rgba(2,6,23,0.12),rgba(2,6,23,0.92))]" />

      <div className="relative z-10 flex h-full flex-col gap-3">
        <div className="flex items-start justify-between gap-3">
          <div>
            <div className="text-[11px] font-black uppercase text-lime-200">Grupo {group.number} — {group.court}</div>
            <h2 className="mt-1 text-2xl font-black uppercase leading-none text-white 2xl:text-3xl">{group.shortName}</h2>
            <div className="mt-1 text-xs font-bold uppercase text-slate-300">{group.sponsor}</div>
          </div>
  ...
  return (
    ...
    >
      <div className="grid grid-cols-[auto_1fr] gap-2">
        <div className={cn("flex h-9 w-9 items-center justify-center rounded-md text-base font-black", row.classifying ? "bg-lime-300 text-slate-950" : "bg-white/10 text-white")}>
          {row.position}º
        </div>
        <div className="min-w-0">
          <div className="break-words text-sm font-black uppercase leading-tight text-white 2xl:text-base">{row.pair.name}</div>
          <div className="mt-1">
            <span className={cn("rounded-md px-2 py-0.5 text-[9px] font-black uppercase", row.classifying ? "bg-lime-300 text-slate-950" : "bg-white/10 text-slate-300")}>
...
function TinyStat({ label, value, good = false, bad = false }: { label: string; value: number | string; good?: boolean; bad?: boolean }) {
  return (
    <div className="rounded-md border border-white/10 bg-white/[0.045] px-1.5 py-1.5">
      <div className={cn("text-sm font-black", good ? "text-lime-300" : bad ? "text-red-200" : "text-white")}>{value}</div>
...
function StatusPill({ live, label }: { live: boolean; label: string }) {
  return (
    <span
      className={cn(
        "inline-flex min-h-9 items-center justify-center gap-2 rounded-md px-3 py-1.5 text-[10px] font-black uppercase ring-1",
        live
